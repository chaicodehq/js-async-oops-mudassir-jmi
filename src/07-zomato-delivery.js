...
export function confirmOrder(order) {
  return new Promise((resolve, reject) => {
    if (!order || typeof order.orderId !== 'number' || order.status !== "placed") {
...
export function processMultipleOrders(orderList) {
  ...
  const promises = orderList.map(orderData => 
    processDelivery(orderData.restaurant, orderData.items)
  );
